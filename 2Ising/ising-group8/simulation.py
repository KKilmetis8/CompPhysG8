...
if c.kind == 'single':
    energies, avg_mags, converged = f.metropolis(p.init_grid, c.MC_steps * p.MC_step, c.temperature, init_energy,
...
if c.kind == 'sweep':
    ...
    for temp in tqdm(c.temperatures, desc='Metropolis simulations') if c.loud else c.temperatures:
        energies, avg_mags, converged = f.metropolis(p.init_grid, c.MC_steps * p.MC_step, temp, init_energy,
                                                      c.J_coupling, c.H_ext, buffer = c.buffer)
